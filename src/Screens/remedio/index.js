...
export default function Remedio() {
  ...
  const adicionarRemedio = () => {
    ...
    setRemedios([...remedios, remedioParaAdicionar]);
    ...
    Alert.alert('Sucesso!', `${remedioParaAdicionar.nome} agendado para ${remedioParaAdicionar.horario}`);
  };

  const removerRemedio = (id) => {
    setRemedios(remedios.filter(rem => rem.id !== id));
  };
        ...
        {remedios.length > 0 ? (
          remedios.map(remedio => (
            <View key={remedio.id} style={estilos.cardRemedio}>
              {remedio.imagem ? (
                <Image source={{ uri: remedio.imagem }} style={estilos.imagemRemedio} />
              ) : (
                <View style={estilos.iconeRemedio}>
                  <Icon name="medication" size={30} color="#5A00DD" />
                </View>
              )}
              
              <View style={estilos.infoRemedio}>
                <Text style={estilos.nomeRemedio}>{remedio.nome}</Text>
                
                <View style={estilos.linhaDetalhe}>
                  <Icon name="schedule" size={18} color="#5A00DD" style={estilos.iconeDetalhe} />
                  <Text style={estilos.textoDetalhe}>{remedio.horario}</Text>
                </View>
                
                {remedio.dosagem && (
                  <View style={estilos.linhaDetalhe}>
                    <Icon name="science" size={18} color="#5A00DD" style={estilos.iconeDetalhe} />
                    <Text style={estilos.textoDetalhe}>{remedio.dosagem}</Text>
                  </View>
                )}
                
                {remedio.frequencia && (
                  <View style={estilos.linhaDetalhe}>
                    <Icon name="repeat" size={18} color="#5A00DD" style={estilos.iconeDetalhe} />
                    <Text style={estilos.textoDetalhe}>{remedio.frequencia}</Text>
                  </View>
                )}
              </View>
          ...
          <View style={estilos.listaVazia}>
            ...
            <Text style={estilos.textoListaVazia}>Nenhum remédio agendado</Text>
      ...
      >
        <View style={estilos.containerModal}>
          ...
          <ScrollView style={estilos.conteudoModal}>
            <TouchableOpacity style={estilos.botaoImagem} onPress={tirarFoto}>
              {novoRemedio.imagem ? (
                <Image source={{ uri: novoRemedio.imagem }} style={estilos.previaImagem} />
              ) : (
                <View style={estilos.placeholderImagem}>
                  <Icon name="add-a-photo" size={30} color="#5A00DD" />
                  <Text style={estilos.textoBotaoImagem}>Foto do remédio (opcional)</Text>
                </View>
              )}
            </TouchableOpacity>

            <View style={estilos.grupoInput}>
              <Text style={estilos.rotuloInput}>Nome do Remédio *</Text>
              <TextInput
                style={estilos.input}
                placeholder="Ex: Paracetamol"
                value={novoRemedio.nome}
                onChangeText={(texto) => setNovoRemedio({...novoRemedio, nome: texto})}
              />
            </View>
            ...
            <View style={estilos.grupoInput}>
              <Text style={estilos.rotuloInput}>Dosagem (opcional)</Text>
              <TextInput
                style={estilos.input}
                placeholder="Ex: 500mg"
                value={novoRemedio.dosagem}
                onChangeText={(texto) => setNovoRemedio({...novoRemedio, dosagem: texto})}
              />
            </View>
            ...
            >
              <Text style={estilos.textoBotaoSalvar}>Salvar Remédio</Text>
            </TouchableOpacity>
          </ScrollView>
        </View>
      </Modal>
